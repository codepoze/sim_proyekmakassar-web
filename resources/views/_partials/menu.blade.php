...
<li class="nav-item {{ ($row->jenis === 'multi' && Request::segment(2) == $url_menu_head ? 'open' : (Request::segment(2) == $url_menu_head ? 'active' : '')) }}">
    <a href="{{ URL::to($row->jenis === 'multi' ? '#' : 'admin' . $row->path) }}" class="d-flex align-items-center" data-id_menu_head="{{ $row->id_menu_head }}">
    ...
    <ul class="menu-content">
        ...
        <li>
            <a href="{{ URL::to('admin' . $row->path . $row3->path) }}" class="{{ (Request::segment(2) == $url_menu_head && Request::segment(3) == $url_menu_body) ? 'active' : '' }}">
