<x-admin-layout title="{{ $title }}">
            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-datatable">
                        <table class="table table-striped table-bordered" id="tabel-konsultan-dt" style="width: 100%;">
                        </table>
                    </div>
                </div>
            </div>
    ...
    <div id="modal-add-upd" class="modal fade text-start" tabindex="-1" role="dialog" data-bs-backdrop="static" data-bs-keyboard="false">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                ...
                <!-- begin:: untuk form -->
                <form id="form-add-upd" class="form form-horizontal" action="{{ route_role('admin.konsultan.save') }}" method="POST">
    ...
    <script>
        ...
        let untukTabel = function() {
            table = $('#tabel-konsultan-dt').DataTable({
                serverSide: true,
                responsive: true,
                processing: true,
                ...
                ajax: "{{ route_role('admin.konsultan.get_data_dt') }}",
        ...
        let untukUbahData = function() {
            $(document).on('click', '#upd', function(e) {
                ...
                $.ajax({
                    type: 'POST',
                    dataType: 'json',
                    url: "{{ route_role('admin.konsultan.show') }}",
                    data: {
                        id: ini.data('id')
                    },
                    beforeSend: function() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        Swal.fire({
                            ...
                            $.ajax({
                                type: "post",
                                url: "{{ route_role('admin.konsultan.del') }}",
                                dataType: 'json',
                                data: {
                                    id: ini.data('id'),
