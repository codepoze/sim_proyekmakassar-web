...
class PaketController extends Controller
{
    ...
    public function det()
    {
        ...
        $data = [
            'id'    => $id,
            'paket' => Paket::find(my_decrypt($id)),
        ];

        return Template::load('admin', 'Detail', 'paket', 'det', $data);
    }
    ...
    public function get_data_dt()
    {
        ...
        return DataTables::of($data)
            ...
            ->addColumn('action', function ($row) {
                return '
                    <a href="' . route_role('admin.paket.det', ['id' => my_encrypt($row->id_paket)]) . '" class="btn btn-sm btn-relief-info"><i data-feather="info"></i>&nbsp;Detail</a>&nbsp;
                    <button type="button" id="upd" data-id="' . my_encrypt($row->id_paket) . '" class="btn btn-sm btn-relief-primary" data-bs-toggle="modal" data-bs-target="#modal-add-upd"><i data-feather="edit"></i>&nbsp;<span>Ubah</span></button>&nbsp;
                    <button type="button" id="del" data-id="' . my_encrypt($row->id_paket) . '" class="btn btn-sm btn-relief-danger"><i data-feather="trash"></i>&nbsp;<span>Hapus</span></button>
                ';
            })
            ->make(true);
    }
    ...
    {
        $response = Paket::find(my_decrypt($request->id));
