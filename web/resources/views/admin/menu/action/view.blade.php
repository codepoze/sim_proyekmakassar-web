...
<div id="modal-add-upd" class="modal fade text-start" tabindex="-1" role="dialog" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <!-- begin:: untuk form -->
            <form id="form-add-upd" class="form form-horizontal" action="{{ route('admin.menu.action.save') }}" method="POST">
...
<script>
    ...
    let untukTabel = function() {
        table = $('#tabel-menu-action-dt').DataTable({
            ...
            ajax: "{{ route('admin.menu.action.get_data_dt') }}",
    ...
    let untukUbahData = function() {
        $(document).on('click', '#upd', function(e) {
            ...
            $.ajax({
                type: 'POST',
                dataType: 'json',
                url: "{{ route('admin.menu.action.show') }}",
                data: {
                    id: ini.data('id')
                },
                beforeSend: function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    Swal.fire({
                        ...
                        $.ajax({
                            type: "post",
                            url: "{{ route('admin.menu.action.del') }}",
                            dataType: 'json',
                            data: {
                                id: ini.data('id'),
    ...
    let untukSelectMenuHead = function() {
        $.get("{{ route('admin.menu.head.get_all') }}", function(response) {
